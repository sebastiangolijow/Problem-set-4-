...
RGBTRIPLE get_blurred_pixel(int i, int j, int height, int width, RGBTRIPLE image[height][width])
{
    ...
    blurred_pixel.rgbtRed = round((float) redValue / num0fvalidpixels); 
    blurred_pixel.rgbtBlue = round((float) blueValue / num0fvalidpixels);
    blurred_pixel.rgbtGreen = round((float) greenValue / num0fvalidpixels);
